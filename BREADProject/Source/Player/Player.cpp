	...
	namespace FrameWork
	{
		...
		void PlayerActor::Update(const f32& dt)
		{
			...
			//modelの更新
			{
				...
				playerModel->UpdateTransform(dt / 60.0f);//モデルの更新
			}
			...
			//CCDIKの更新
			{
				...
				Transform*  leftT   = leftFootTargetActor->GetComponent<Transform>();
				{
					...
					ccdik->order.at(0)->setTargetPos(GetLocation(leftT->GetWorldTransform()));//CCDIKのTargetの更新
				...
				Transform*    rightT     = rightFootTargetActor->GetComponent<Transform>();
				{
					...
					ccdik->order.at(1)->setTargetPos(GetLocation(rightT->GetWorldTransform()));//CCDIKのTargetの更新
